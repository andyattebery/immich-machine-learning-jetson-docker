...
├── Dockerfile.builder     # builder stage only; merged with upstream prod stage
├── Dockerfile.generated   # auto-generated by scripts/generate_dockerfile.py — do not edit
...
│       Dockerfile.builder + upstream immich prod stage      │
│       → Dockerfile.generated                               │
│                                                            │
│  6. build-immich (docker build -f Dockerfile.generated)    │
│     ┌────────────────────────────────────────────────────┐ │
│     │  builder stage                                     │ │
...
- **builder** stage (defined in [Dockerfile.builder](Dockerfile.builder)) uses the full ORT image: needs `python3.11` and `/opt/onnxruntime*.whl`
- **prod** stage (taken from upstream immich, merged into [Dockerfile.generated](Dockerfile.generated)) uses the cudastack image: needs only the CUDA/cuDNN/TRT runtime libraries
...
7. Generate `Dockerfile.generated` by merging `Dockerfile.builder` with the upstream immich prod stage at the resolved version
8. Build the Immich ML Docker image from `Dockerfile.generated` using the full onnxruntime image as builder base and the cudastack image as prod base
...
| `generate-dockerfile`    | Generate `Dockerfile.generated` from `Dockerfile.builder` + upstream immich prod stage  |
