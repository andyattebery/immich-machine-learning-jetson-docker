...
├── Dockerfile.immich-builder     # builder stage only; merged with upstream prod stage
├── Dockerfile.immich-generated   # auto-generated by scripts/generate_dockerfile.py — do not edit
├── docker-compose.yaml           # runs the ML service
├── .env.example                  # example environment variables
├── mise.toml                     # optional: pins Python toolchain for generate_dockerfile.py and jetson-containers venv
├── .gitignore
├── README.md                     # this file
├── scripts/                      # generate_dockerfile.py + Tier 1/2 test scripts
└── vendor/                       # cloned external repos (gitignored)
...
│                                                            │
│  2. checkout                                               │
│     git clone --depth 1 --branch v2.7.4 immich → vendor/   │
│                                                            │
...
│     scripts/generate_dockerfile.py merges                  │
│       Dockerfile.immich-builder + upstream immich prod     │
│       → Dockerfile.immich-generated                        │
│                                                            │
│  6. build-immich                                           │
│     docker build -f Dockerfile.immich-generated            │
│     ┌────────────────────────────────────────────────────┐ │
│     │  builder stage                                     │ │
...
│     ├────────────────────────────────────────────────────┤ │
│     │  prod stage (from upstream immich Dockerfile)      │ │
...
│     │  - immich_ml source from vendor/immich             │ │
...
- **builder** stage (defined in [Dockerfile.immich-builder](Dockerfile.immich-builder)) uses the full ORT image: needs `python3.11` and `/opt/onnxruntime*.whl`
- **prod** stage (taken from upstream immich, merged into [Dockerfile.immich-generated](Dockerfile.immich-generated)) uses the cudastack image: needs only the CUDA/cuDNN/TRT runtime libraries
...
Requires `curl` and `jq` for GitHub API version resolution, plus `python3` with `venv` support (the Makefile auto-installs `python3-venv` if missing). The Makefile handles everything else automatically — but two host-level settings must be configured before the first build.

[mise](https://mise.jdx.dev) is an **optional** way to manage Python and its dependencies. If `mise` is on your `PATH`, the Makefile uses `mise exec -- python` and the [mise.toml](mise.toml) pins Python 3; otherwise it falls back to the system `python3`.
...
Once `sudo make setup-host` has been run (see [Prerequisites](#prerequisites)), the full pipeline is a single command:
...
First run takes 1–2 hours (ORT source compilation). Subsequent runs skip the ORT build if a matching `onnxruntime-jetson:<version>-py311` image already exists.

The Makefile will:
1. Resolve `latest` to the actual release tag via GitHub API (`curl`, no `gh` CLI)
2. Shallow-clone immich at that tag into `vendor/immich/`
3. Parse `pyproject.toml` to detect the required `onnxruntime-gpu` version
4. Clone (or refresh) jetson-containers into `vendor/jetson-containers/` and, if not cached, create its Python venv
5. Check if `onnxruntime-jetson:<version>-py311` exists locally; if not, compile it via `jetson-containers build python:3.11 onnxruntime:<version>` and tag the `cudastack_standard` intermediate as `onnxruntime-cudastack-jetson:<version>-py311`
6. Generate `Dockerfile.immich-generated` by merging `Dockerfile.immich-builder` with the upstream immich prod stage at the resolved version
7. Build the Immich ML Docker image from `Dockerfile.immich-generated` using the full onnxruntime image as builder base and the cudastack image as prod base
...
| `build`                  | Full pipeline: resolve → checkout → onnxruntime → generate-dockerfile → ML image        |
...
| `generate-dockerfile`    | Generate `Dockerfile.immich-generated` from `Dockerfile.immich-builder` + upstream immich prod stage  |
| `build-wyoming-whisper`  | Build `wyoming-whisper` on top of the ORT base (see [docs/wyoming.md](docs/wyoming.md)) |
| `build-wyoming-piper`    | Build `wyoming-piper` on top of the ORT base (see [docs/wyoming.md](docs/wyoming.md))   |
| `up`                     | `docker compose up -d`                                                                  |
| `down`                   | `docker compose down`                                                                   |
| `clean`                  | Remove `vendor/` and the ML image (preserves onnxruntime image)                         |
...
```bash
# Resolve the actual tag (e.g. onnxruntime-jetson:1.23.2-py311) via `make help`:
docker run --rm onnxruntime-jetson:<version>-py311 \
...
```bash
docker create --name ort-pin onnxruntime-jetson:<version>-py311 true
```
...
If the new immich version requires a newer `onnxruntime-gpu`, the Makefile detects the bump, finds no matching `onnxruntime-jetson:*-py311` image, and rebuilds ORT automatically (adding another 1–2 hours).
...
There are no pre-built ORT tarballs for JetPack 6.2 / CUDA 12.6 at [apt.jetson-ai-lab.io](https://apt.jetson-ai-lab.io/jp6/cu126/). Every version (`>=1.23.2`) falls back to full source compilation, which takes **1–2 hours** on Jetson hardware. This is a one-time cost per ORT version; subsequent `make build` runs skip it if the tagged image already exists.
...
## Other jetson-containers packages

The ORT base image this project builds can be reused for other GPU-accelerated services on the same Jetson, skipping the 1–2h ORT recompile.

- **Wyoming voice services** — `wyoming-whisper` (speech-to-text) and `wyoming-piper` (text-to-speech) for Home Assistant or other Wyoming clients. See [docs/wyoming.md](docs/wyoming.md) for the `make build-wyoming-whisper` / `make build-wyoming-piper` targets, manual build equivalent, and known upstream issues.
...
- [apt.jetson-ai-lab.io](https://apt.jetson-ai-lab.io/jp6/cu126/) — Pre-built JetPack 6.2 / CUDA 12.6 wheels (no ORT `>=1.23.2` available; source compilation required)
- [astral-sh/uv](https://github.com/astral-sh/uv) — Python package installer used in the builder stage
- [mise](https://mise.jdx.dev) — Optional Python toolchain manager auto-detected by the Makefile
